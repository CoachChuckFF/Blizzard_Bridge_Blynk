...
uint8_t _dmx_state = DMX_STATE_IDLE;
...
static void uart_rx_intr_handler_default(void *param)
{
    ...
    while(uart_intr_status != 0x0) {
        ...
        if(uart_num == DMX_UART && uart_intr_status & UART_TX_DONE_INT_ST_M)
        {
          UART_ENTER_CRITICAL_ISR(&uart_spinlock[uart_num]);
          uart_reg->int_ena.tx_done = 0;
          uart_reg->int_clr.tx_done = 1;
          if(_dmx_state == DMX_STATE_BREAK)
            _dmx_state = DMX_STATE_START;
          if(_dmx_state == DMX_STATE_DATA)
            _dmx_state = DMX_STATE_BREAK;
          uart_reg->int_ena.txfifo_empty = 1; //re-enable tx empty
          ...
          if(uart_num == DMX_UART)
          {
            ...
            switch(_dmx_state)
            {
              case DMX_STATE_START:
                ...
                WRITE_PERI_REG(UART_FIFO_AHB_REG(uart_num), _start_byte); //send start byte

              break;
              case DMX_STATE_DATA:
              WRITE_PERI_REG(UART_FIFO_AHB_REG(uart_num), _dmx_data[_current_slot++]);
              if(_current_slot >= DMX_MAX_SLOTS)
              {
                uart_enable_intr_mask(DMX_UART, UART_TX_DONE_INT_ENA_M);
              ...
              WRITE_PERI_REG(UART_FIFO_AHB_REG(uart_num), 0); //send break byte
              uart_enable_intr_mask(DMX_UART, UART_TX_DONE_INT_ENA_M);
          ...
          uart_event.type = UART_BREAK;
            ...
            if(uart_num == DMX_UART && !_tx_running) //modified isr
            {
              ...
              _incoming_byte = uart_reg->fifo.rw_byte;
...
esp_err_t uart_driver_install(uart_port_t uart_num, int rx_buffer_size, int tx_buffer_size, int queue_size, QueueHandle_t *uart_queue, int intr_alloc_flags)
{
    ...
    if(uart_num == DMX_UART)
    {
      ...
      uart_intr.rx_timeout_thresh = 100;//UART_TOUT_THRESH_DEFAULT;
    ...
    else
    {
      uart_intr.intr_enable_mask = UART_RXFIFO_FULL_INT_ENA_M
                              ...
                              | UART_BRK_DET_INT_ENA_M
                              | UART_PARITY_ERR_INT_ENA_M;
      uart_intr.rxfifo_full_thresh = UART_FULL_THRESH_DEFAULT;
